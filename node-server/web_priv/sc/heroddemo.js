...
function upd() {
 ...
 console.log("php/summaryres.php?setup="+$('input[name=setup]:checked', '#setups').val()+"&bu="+minidaqbu)
...
function runcmd(cmd) {
  ...
  $.getJSON("php/summaryres.php?setup="+$('input[name=setup]:checked', '#setups').val()+"&bu="+minidaqbu,function(data) {
    ...
    if (confirm(message1 + "Are you sure that you want to run command "+cmd.toUpperCase()
        ...
        $("#"+cmd+"btn").hide();
        $("#"+cmd+"btnrunning").show();
        $.getJSON("php_priv/runcommanddemo.php?setup="+$('input[name=setup]:checked', '#setups').val()+"&cmd="+cmd+"&bu="+minidaqbu,
           function(data) {
